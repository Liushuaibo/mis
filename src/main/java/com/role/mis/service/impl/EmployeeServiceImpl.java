...
import com.role.mis.enums.EmployeeQueryCondition;
import com.role.mis.mapper.DeptMapper;
import com.role.mis.mapper.EmpRoleMapper;
import com.role.mis.mapper.EmployeeMapper;
import com.role.mis.mapper.RoleMapper;
import com.role.mis.pojo.*;
import com.role.mis.qo.EmployeeQuery;
import com.role.mis.service.EmployeeService;
...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public List<Employee> findByPageQuery(int pageNum, int pageSize, EmployeeQuery employeeQuery) {
        ...
        // 根据员工姓名进行模糊查询
        if (employeeQuery != null && employeeQuery.getCondition() == EmployeeQueryCondition.EMPLOYEE_ENAME.getCondition()) {
            criteria.andEnameLike("%"+employeeQuery.getKeyword()+"%");
        }

        // 根据部门名称对部门进行模糊查询, 然后根据查询到的结果id查询该部门中的员工
